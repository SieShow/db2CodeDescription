    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (lineText && lineText.length > 2) {
        ...
        if (this.checkIfIsCode(lineText)) {
          ...
          errorObj.$destination = splited[0];
          ...
          errorObj.appendExplanation(lineText.replace(splited[0], ''));
          this.statusForExplanation = SetStatus.READING;

          if (i + 1 < lines.length && this.isInformationFinished(lines[i + 1].split(' '))) {
          ...
          errorObj.appendSystemAction(lineText.replace(splited[0], ''));
          ...
          errorObj.appendProgrammerResponse(lineText.replace(splited[0], ''));
